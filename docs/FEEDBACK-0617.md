...
- [x] **G1 — Specific, profile-grounded recommendations.** Make chat/council/recommendations
...
- [x] **G2 — "What has it learned about me?"** Surface the learned per-user profile
...
- [x] **G3 — Incognito toggle.** A visible control (top-right, like other apps) that
...
- [x] **P1 — Processes don't stop on navigation.** A convened council (and a long
...
- [x] **P2 — Sidebar process indicator.** A live "N processes" indicator in the
...
- [x] **P3 — Council canvas shows the LATEST running council.** When results stream
...
- [x] **C2 — Reflect global/local split in the UI.** Group the context panel into a
...
- [x] **C3 — Clickable decisions.** Each decision in the list opens its full content
  (name + verdict/detail), not just the first. Today you can't click through.

---

## Theme 5 — Surface fixes (concrete bugs)
- [x] **S1 — Insights needs two clicks.** Clicking Insights should land on Insights
...
- [x] **S2 — Usage always blank.** Usage panel shows nothing; make it populate as
  close to realtime as possible after each chat/turn.
- [x] **S3 — Benchmark draft "failure" is opaque.** "Drafted 0/1 — failed: Wealth"
...
- [x] **S4 — Remove Ideal-state editor from Loops.** Loops shouldn't host a "draft
...
- [x] **S5 — App configuration clarity.** A "not-configured" app (e.g. PayPal) gives
...
- [x] **D1 — On/Off toggle everywhere (no Start/Stop text).** Replace all Start/Stop
  text (daemons, reminders, backups, etc.) with the toggle "peel" switch. Recurring
  ask; sweep every call site.
- [x] **D2 — Remove duplicate Ideal State from Memory engine.** The Memory-engine page
  repeats Ideal State that already sits right above it. Remove the dup.
- [x] **D3 — Access protection: two compact columns.** App lock | Vault encryption
  side by side, minimal, instead of two tall stacked blocks.
- [x] **D4 — Simplify Workspace / Vault + Backups.** Keep the top "Vault" section
...
- [x] **D5 — No em dashes in any design/UI copy.** Sweep components for "—" in
  visible strings; replace with ":" / "," / "-".
- [x] **D6 — Rename context-window button.** "Compact — summarize & continue" →
  "Compact & Summarize" (fixes the em dash + the word wrap).

---

## Theme 7 — Formatting
- [x] **F1 — "Questions worth asking" panel.** Format the 5-question panel nicely
